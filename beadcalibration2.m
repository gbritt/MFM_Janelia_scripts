function beadcalibration2(planes,thresh,edg,objmag,multicolor,filename,pathname,flip)
...
if multicolor
    ...
    [beadsg,imgsg,zstepsizeg,zmaxg] = beadfinder2(planes,thresh,edg,filenameg,pathnameg,flip(1));
    [beadsr,imgsr,zstepsizer,zmaxr] = beadfinder2(planes,thresh,edg,filenamer,pathnamer,flip(2));
    ...
    if ~strcmp(beadcheck,'n')
        ...
        save([pathnamer filesaver],'dr','intscorrectr','fitparamsr','psfsr','tformr','zstepsizer','objmag');
    else
        error('Beads no good')
    end
else
    [beads,imgs,zstepsize,zmax] = beadfinder2(planes,thresh,edg,filename,pathname,max(flip));
    beadcheck = input('Beads ok? (y/n)','s');
    if ~strcmp(beadcheck,'n')
