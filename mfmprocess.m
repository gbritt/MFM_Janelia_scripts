...
% Last Modified by GUIDE v2.5 18-Oct-2016 13:34:41
...
cond1 = exist('greencalib','var') && exist('greenimgpath','var') && exist('greenimgfile','var');
cond2 = exist('redcalib','var') && exist('redimgpath','var') && exist('redimgfile','var');
...
if cond1 && ~cond2
    ...
    for a = 1:length(greenimgfile)
        mfmwrite4(greenimgfile{a},greenimgpath,intscorrect,tform,psfs,intcorrection,process,radius,numplanes,deconviter,zstepsize,d,patternlengthg,framekeepg,suffixg,flip);
    end
...
elseif cond2 && ~cond1
    ...
    for a = 1:length(redimgfile)
        mfmwrite4(redimgfile{a},redimgpath,intscorrect,tform,psfs,intcorrection,process,radius,numplanes,deconviter,zstepsize,d,patternlengthr,framekeepr,suffixr,flip);
    end
elseif cond1 && cond2
    ...
    for a = 1:length(greenimgfile)
        mfmwrite4(greenimgfile{a},greenimgpath,intscorrectg,tformg,psfsg,intcorrection,process,radius,numplanes,deconviter,zstepsizeg,dg,patternlengthg,framekeepg,suffixg,flipg);
    end
    
    for a = 1:length(redimgfile)
        mfmwrite4(redimgfile{a},redimgpath,intscorrectr,tformr,psfsr,intcorrection,process,radius,numplanes,deconviter,zstepsizer,dr,patternlengthr,framekeepr,suffixr,flipr);
    end
else
